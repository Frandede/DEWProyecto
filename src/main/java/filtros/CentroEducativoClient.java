...
public class CentroEducativoClient {
    ...
    public static String login(String dni, String pass) {
        ...
        try {
            ...
            // Construimos payload JSON: { "dni": "...", "password": "..." }
            String payload = gson.toJson(Map.of("dni", dni, "password", pass));
            ...
            // Guardamos la cookie (JSESSIONID) que envía el servidor
    ...
    public static String getAlumnoPorDNI(String dni, String key) {
        ...
        try {
            String urlStr = String.format("%s/alumnos/%s/?key=%s",
                ...
                URLEncoder.encode(dni, StandardCharsets.UTF_8.name()),
                URLEncoder.encode(key, StandardCharsets.UTF_8.name()));
            System.out.println("URL getAlumnoPorDNI: " + urlStr);
    ...
    public static List<AlumnoAsignatura> getAlumnosPorAsignatura(String acronimo, String dniProfesor, String key) {
        System.out.println("Obteniendo alumnos para asignatura: " + acronimo + " (Profesor: " + dniProfesor + ", Key: " + key + ")");
        HttpURLConnection con = null;
        try {
            String urlStr = String.format("%s/asignaturas/%s/alumnos?key=%s",
                ...
                URLEncoder.encode(key, StandardCharsets.UTF_8.name()));
            System.out.println("URL getAlumnosPorAsignatura: " + urlStr);
    ...
    public static boolean actualizarNota(String asignatura, String dniAlumno, Float nota, String dniProfesor, String key) {
        ...
        try {
            ...
            // El script Bash hace PUT con Content-Type: application/json
            con.setRequestProperty("Content-Type", "application/json; charset=UTF-8");
            ...
            // Enviar únicamente el número de nota, tal cual (por ejemplo "7.5")
            try (OutputStream os = con.getOutputStream()) {
                String rawNota = nota.toString();
                os.write(rawNota.getBytes(StandardCharsets.UTF_8));
            }
