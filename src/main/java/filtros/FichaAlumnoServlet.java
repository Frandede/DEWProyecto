...
public class FichaAlumnoServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        // 1) Comprobamos sesión del alumno
        ...
        // 3) Construimos la página HTML con el diseño unificado
        ...
        try (PrintWriter out = resp.getWriter()) {
             ...
             out.println("  <title>Ficha de " + name + "</title>");
             ...
             out.println("    .subject-list li { background: rgba(0,0,0,0.15); margin-bottom: 0.5rem; border-radius: 8px; font-size: 0.9rem; transition: background-color 0.2s; }");
             out.println("    .subject-list li:hover { background: rgba(0,0,0,0.25); }");
             out.println("    .subject-list a { display: block; padding: 0.75rem; color: var(--primary-color); text-decoration: none; }");
             ...
             out.println("        </div>");
             out.println("        <a href='" + req.getContextPath() + "/alumno/asignaturas?dni=" + dni + "' class='btn-glass'>Ver Mis Asignaturas</a>");
             out.println("      </div>");

             out.println("      <div class='ficha-body'>");
             out.println("        <div class='photo-column'>");
             out.println("          <img id='student-photo' src='" + req.getContextPath() + "/images/alumnos/" + dni + ".png' "
             ...
             out.println("          <h4 style='margin-top: 2rem;'>Mis Asignaturas (clic para ver nota):</h4>");
             if (asignaturas == null || asignaturas.isEmpty()) {
                 out.println("<p>No estás matriculado en ninguna asignatura.</p>");
             } else {
                 out.println("            <ul class='subject-list'>");
                 for (Asignatura a : asignaturas) {
                     String acr = a.getAsignaturasDeAlumno();
                     out.println("              <li><a href='" + req.getContextPath() + "/alumno/nota?asignatura=" + acr + "'>" + acr + "</a></li>");
                 }
                 out.println("            </ul>");
             }
             out.println("        </div>");
